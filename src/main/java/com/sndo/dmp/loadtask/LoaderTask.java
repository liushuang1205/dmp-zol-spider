...
public class LoaderTask {
    ...
    private int query(Integer num) throws InterruptedException {
    ...
    private boolean isExist(Document doc) {
        ...
        if (resultDoc == null || resultDoc.isEmpty()) {
            ...
            //boolean isAndroid = isAndroid(doc);
    ...
    private Boolean isAndroid(Document doc){
        if(doc.get("android") != null){
            return true;
        } else if(doc.get("ios") != null){
            return false;
        }
        return null;
    }
...
//            models.add(new UpdateOneModel<>(filterDoc, new Document("$set", doc), updateOptions));
    ...
    public int start(Integer num) throws InterruptedException {
